...
#if Py_WIN8APP
    /* Declare that WinRT uses UTF-8 for all IO. */
...
{
#if Py_WIN8APP
	/* Cannot create subprocesses in the first place. */
	return 0;
#elif defined(MS_WINDOWS)
    HANDLE handle;
    DWORD flags;
...
set_inheritable(int fd, int inheritable, int raise, int *atomic_flag_works)
{
    ...
    if (!inheritable)
        /* Nothing to do */
        return 0;
    PyErr_SetString(PyExc_NotImplementedError, "descriptor inheritance is irrelevant on Windows 8");
    return -1;
#elif defined(MS_WINDOWS)
    if (!_PyVerify_fd(fd)) {
        if (raise)
