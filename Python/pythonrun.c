...
_Py_IDENTIFIER(ps1);
_Py_IDENTIFIER(ps2);
/* stdin is a macro on Windows Runtime */
_Py_static_string(PyId_stdin, "stdin");
_Py_static_string(PyId_stdout, "stdout");
_Py_static_string(PyId_stderr, "stderr");
...
#if Py_WIN8APP
	/* Do not try to create streams for fd 0/1/2 on WinRT, as they will not be valid. */
#else
...
#ifndef NDEBUG
        OutputDebugStringA(((PyTypeObject*)exception)->tp_name);
    {
        PyObject *msg = PyObject_Repr(v);
        OutputDebugString(PyUnicode_AsUnicodeCopy(msg));
    }
    {
        void print_exception_recursive(PyObject *f, PyObject *value, PyObject *seen);
        extern PyTypeObject PyStringIO_Type;
        PyObject *msg;
        PyObject *seen = PySet_New(NULL);
        PyObject *stream = PyObject_CallFunction((PyObject*)&PyStringIO_Type, "");
        print_exception_recursive(stream, v, seen);
        msg = PyObject_CallMethod(stream, "getvalue", "");
        OutputDebugString(PyUnicode_AsUnicodeCopy(msg));
    }
#endif
    if (hook) {
...
Py_FatalError(const char *msg)
{
...
#ifdef MS_WINDOWS
    {
        ...
        for( i=0; i<=len; ++i)
            buffer[i] = msg[i];
#if !Py_WIN8APP || defined(_DEBUG)
        OutputDebugStringW(L"Fatal Python error: ");
        OutputDebugStringW(buffer);
        OutputDebugStringW(L"\n");
#endif
    }
#if defined(_DEBUG) && !Py_WIN8APP
    DebugBreak();
#endif
#endif /* MS_WINDOWS */
    abort();
...
{
#if Py_WIN8APP
	/* Cannot do stack checks on Windows Runtime */
	return 0;
#else
    __try {
