...
AllocNonRecursiveMutex()
{
    ...
    return CreateSemaphoreExW(NULL, 1, 1, NULL, 0, EVENT_ALL_ACCESS);
#else
...
EnterNonRecursiveMutex(PNRMUTEX mutex, DWORD milliseconds)
{
    return WaitForSingleObjectEx(mutex, milliseconds, TRUE);
}
...
{
	PyErr_SetString(PyExc_NotImplementedError, "Thread creation is not supported on this system");
	return -1;
}
